...
with DAG(
    ...
    def get_partition_value():
        ...
        - ods:
            ...
            - 有partition:, 指定empty取全部partition, 或指定特定partition, 可指定特定區間, ex: 202502~202506
        - tfd:
    ...
    def check_partition(**kwargs):
        context = get_current_context()
        tfd_to_cp = context["dag_run"].conf.get("tfd_to_cp")
        if tfd_to_cp == '"True"':
            ti = kwargs['ti']
            hook = S3Hook()
    ...
    etl_trigger = GlueJobOperator(
        ...
        create_job_kwargs={
            ...
            "DefaultArguments": {
                ...
                "--extra-py-files": f"s3://{{label}}-s3-glue-sg-{{account_id}}/libraries/crmcplhp03/pypackage/tfd_module.zip",
            }
        },
        update_config=True,
        script_args={
            '--db_table': '{% raw %}{{ dag_run.conf.get("db_table") or params.get("db_table", "default_db_table") }}{% endraw %}',
            '--tfd_to_cp': '{% raw %}{{ dag_run.conf.get("tfd_to_cp") or params.get("tfd_to_cp", "True") }}{% endraw %}',
            '--exec_time': '{% raw %}{{ ti.xcom_pull(task_ids="get_partition_value") }}{% endraw %}'
        },
    )

    manual_trigger = GlueJobOperator(
        task_id="manual_job",
        job_name = f"crmcplhp03-{table_name}",
        iam_role_name='cubaws-GlueIngressDsRole',
        outlets=None,
    ...
    judge_op = judge_manual()
    partition_task >> judge_op >> [manual_trigger, etl_trigger] >> add_tags_task >> check_partition_task >> delete_s3